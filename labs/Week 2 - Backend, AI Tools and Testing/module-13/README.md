# Module 13 — Start here

**Clone + own repo:** [CLONE-AND-OWN-REPO-GUIDE.md](../../CLONE-AND-OWN-REPO-GUIDE.md)

**Topic:** SOAP API Design with Java

| Step | Open this |
| ---- | --------- |
| 1 | After slides → [`exercises/EXERCISES-INDEX.md`](exercises/EXERCISES-INDEX.md), exercises **in order** (see **What you produce** for exact `notes/lab13-*.md` files) |
| 2 | OS guide → [`lab13/LAB-13-WINDOWS.md`](lab13/LAB-13-WINDOWS.md) or [`lab13/LAB-13-MACOS.md`](lab13/LAB-13-MACOS.md) |
| 3 | Full lab → [`lab13/LAB-13-GUIDE.md`](lab13/LAB-13-GUIDE.md) |

**Pre-lab vs lab:** Pre-lab exercises are markdown notes under `examples/module-13-exercises/notes/`. Lab 13 is the graded project (see lab guide paths). Do not merge the two paths.
