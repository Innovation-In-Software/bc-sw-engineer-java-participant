...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\lab46-crm`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab46-crm` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab46-crm`) when the lab reads relative files (`.env`, `application.properties`, logs).
