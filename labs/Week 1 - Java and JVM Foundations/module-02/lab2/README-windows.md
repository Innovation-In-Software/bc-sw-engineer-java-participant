...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\Lab2-JavaSyntax`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\Lab2-JavaSyntax` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/Lab2-JavaSyntax`) when the lab reads relative files (`.env`, `application.properties`, logs).
