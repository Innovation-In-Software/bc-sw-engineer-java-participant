...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\lab1-answers`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab1-answers` when ready.
...
| This lab project | `%USERPROFILE%\java-bootcamp\examples\lab1-answers` |
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab1-answers`) when the lab reads relative files (`.env`, `application.properties`, logs).
