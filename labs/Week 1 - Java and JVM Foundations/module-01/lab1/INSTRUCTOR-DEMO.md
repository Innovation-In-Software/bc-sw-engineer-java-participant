...
**Audience:** Instructor-led, projected to the whole class **during/after the Module 1 PPT**, **before** students start the [pre-lab exercises](../exercises/EXERCISES-INDEX.md) and then [Lab 1](LAB-1-GUIDE.md)
**Duration:** 30–40 minutes total (four demos, ~8–10 minutes each)
**Source:** Expands the four demos listed under "Instructor Demonstration" in the [Week 1 Master Curriculum](../../../../curriculum/Week%201%20-%20Java%20and%20JVM%20Foundations/WEEK-CURRICULUM.md) (Demo 1–4) and the "Instructor Demo" slides in the Module 1 deck, with full runnable code and step-by-step narration.

---

## Purpose

Students will first practice in [Exercises 1–8](../exercises/EXERCISES-INDEX.md), then consolidate in Lab 1 (90–120 minutes including extended work). Before either student activity, the instructor runs the **same kind of programs live**, narrating compilation, bytecode, class loading, heap allocation, and garbage collection so students have a mental model for the exercises — then Lab 1.

**Do not reuse the exact student files.** Exercises use `Hello`, `Person`, and related demos under `module-01-exercises/`. Lab 1 has students build `HelloWorld`, `Calculator`, `Employee`, and `MemoryDemo` under `jvm-compilation-lab/`. This demo uses parallel-but-different classes (`Hello`, `HelloMemory`, `HelloLeak`) so the live walkthrough teaches the same concepts without pre-solving student work.
...
**After the demos:** Send students to the [pre-lab exercises](../exercises/EXERCISES-INDEX.md) first (`module-01-exercises/`), then — only after Exercises 1–8 Pass — to the OS how-to and [Lab 1](LAB-1-GUIDE.md) (`jvm-compilation-lab/`). Remind them Lab programs are deliberately similar to demo/`Hello` ideas — same JVM story, their own graded code.
