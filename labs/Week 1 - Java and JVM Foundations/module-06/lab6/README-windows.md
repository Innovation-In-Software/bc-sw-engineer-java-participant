...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\lab6-answers`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab6-answers` when ready.
...
| This lab project | `%USERPROFILE%\java-bootcamp\examples\lab6-answers` |
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab6-answers`) when the lab reads relative files (`.env`, `application.properties`, logs).
