...
**Other OS:** [macOS guide](README-macos.md) · [IDE conventions](../_IDE-CONVENTIONS.md)

- Pre-lab exercises: [`../exercises/`](../exercises/)
## Prerequisites (Windows)

- [Lab 0 (Windows)](../../module-00/lab0/README-windows.md) complete (JDK 21, Maven when needed, Git)
...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\lab4-gc-snippet`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab4-gc-snippet` when ready.
...
| This lab project | `%USERPROFILE%\java-bootcamp\examples\lab4-gc-snippet` |
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab4-gc-snippet`) when the lab reads relative files (`.env`, `application.properties`, logs).
