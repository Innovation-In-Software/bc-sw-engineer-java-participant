...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\Lab3-BankingSystem`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\Lab3-BankingSystem` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/Lab3-BankingSystem`) when the lab reads relative files (`.env`, `application.properties`, logs).
