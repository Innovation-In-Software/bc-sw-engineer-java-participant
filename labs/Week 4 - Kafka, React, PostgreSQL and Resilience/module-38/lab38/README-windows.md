# Lab 38: PostgreSQL SQL and CRM Query Performance — DBMS_XPLAN, Indexes, Sargable Predicates — Windows
...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\lab38-crm`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab38-crm` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab38-crm`) when the lab reads relative files (`.env`, `application.properties`, logs).
