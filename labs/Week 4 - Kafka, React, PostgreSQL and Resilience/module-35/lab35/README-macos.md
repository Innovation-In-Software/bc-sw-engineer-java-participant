# Lab 35: Integrating React with the Spring CRM API — macOS
...
2. **File → Open…** → `~/java-bootcamp/examples/lab35-crm`  
...
7. **View → Tool Windows → Terminal** → `cd ~/java-bootcamp` then `cd examples/lab35-crm` when ready.
...
| This lab project | `~/java-bootcamp/examples/lab35-crm` |
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab35-crm`) when the lab reads relative files (`.env`, `application.properties`, logs).
