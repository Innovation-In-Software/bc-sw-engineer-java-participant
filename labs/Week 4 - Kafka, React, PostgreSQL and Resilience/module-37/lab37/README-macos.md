# Lab 37: PostgreSQL Design for Customers and Accounts — macOS
...
2. **File → Open…** → `~/java-bootcamp/examples/lab37-crm`  
...
7. **View → Tool Windows → Terminal** → `cd ~/java-bootcamp` then `cd examples/lab37-crm` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/lab37-crm`) when the lab reads relative files (`.env`, `application.properties`, logs).
