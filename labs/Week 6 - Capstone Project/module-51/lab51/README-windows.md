...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp\examples\customer-management-platform`  
...
7. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\customer-management-platform` when ready.
...
2. Green ▶ → **Run**.
3. **Run → Edit Configurations…** → set **Working directory** to the project root (`examples/customer-management-platform`) when the lab reads relative files (`.env`, `application.properties`, logs).
